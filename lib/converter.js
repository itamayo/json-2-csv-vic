...
module.exports = {
    ...
    json2csv: function (array, callback, opts,scope) {
       // console.log("Hemenjson2scv:"+scope);
        buildOptions(opts, function (err, options) { // Build the options
            if (err) {
                ...
                //console.log(scope);
                json2Csv.json2csv(options, array, callback,scope); // Call our internal json2csv function
            }
        });
    },
    ...
    // a callback that will be called with (err, csv) after
    // processing is completed, and optional options
    csv2json: function (csv, callback, opts,scope,t) {
        buildOptions(opts, function (err, options) { // Build the options
            if (err) {
                return callback(err);
            } else {
                //console.log(scope);
                csv2Json.csv2json(options, csv, callback,scope,t); // Call our internal csv2json function
            }
        });
    }
};
