...
var retrieveHeading = function (data) {
    // Returns a function that takes a callback - the function is passed to async.parallel
        ...
        return _.flatten(keys).join(options.DELIMITER.FIELD);
...
var generateCsv = function (data,headings) {
...
module.exports = {
    ...
    json2csv: function (opts, data, callback,scope) {
        ...
        var heading=retrieveHeading(data);
       // console.log(heading);
        var csv=generateCsv(data);
        csv=heading+'\n'+csv;
        callback(null,csv,scope);
